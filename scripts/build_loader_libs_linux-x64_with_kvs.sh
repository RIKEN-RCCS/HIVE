...
function build_kvs {
	...
	CXX=${cxx_compiler} CC=${c_compiler} CFLAGS=${c_flags} CXXFLAGS=${cxx_flags} ${CMAKE_BIN} -DCMAKE_INSTALL_PREFIX=${installdir}/kvs -DKVS_TOOL=OFF -DKVS_SUPPORT_GLUT=ON -DKVS_SUPPORT_QT=OFF -DKVS_SUPPORT_OPENCV=OFF -DKVS_SUPPORT_CUDA=OFF ../KVS && make && make install
