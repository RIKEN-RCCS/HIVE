...
function build_pdmlib {
	cd third_party/
	rm -rf fpzip-1.0.1/
	curl -O https://computation.llnl.gov/projects/floating-point-compression/download/fpzip-1.0.1.tar.gz
	...
	#rm -rf Zoltan_v3.81/
	#rm -rf Zoltan_build/
	#curl -O http://www.cs.sandia.gov/~kddevin/Zoltan_Distributions/zoltan_distrib_v3.81.tar.gz
	...
	#cd ..
	#ZOLTAN_CONFIGURE_PATH=../Zoltan_v3.81/configure

	## git ver.
	rm -rf Trilinos
	git clone https://github.com/trilinos/Trilinos
	ZOLTAN_CONFIGURE_PATH=../Trilinos/packages/zoltan/configure 
