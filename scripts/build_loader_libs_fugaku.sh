...
function build_tp {
	...
	CXX=${cxx_compiler} CC=${c_compiler} CFLAGS=${c_flags} CXXFLAGS=${cxx_flags} ${CMAKE_BIN} -DINSTALL_DIR=${installdir}/TextParser -DCMAKE_TOOLCHAIN_FILE=../TextParser/cmake/Toolchain_Fugaku.cmake -Dwith_MPI=yes -Denable_fapi=yes ../TextParser && make VERVOSE=1 && make install
	if [[ $? != 0 ]]; then exit $?; fi
...
function build_cdmlib {
	...
	CXX=${cxx_compiler} CC=${c_compiler} CFLAGS=${c_flags} CXXFLAGS=${cxx_flags} ${CMAKE_BIN} -DINSTALL_DIR=${installdir}/CDMlib -DCMAKE_TOOLCHAIN_FILE=../CDMlib/cmake/Toolchain_Fugaku.cmake -Dwith_MPI=yes -Dwith_example=yes -Dwith_util=no -Dwith_TP=${installdir}/TextParser -Dwith_CPM=${installdir}/CPMlib -Dwith_NetCDF=/vol0001/apps/oss/spack/opt/spack/linux-rhel8-a64fx/fj-4.2.0/netcdf-c-4.7.3-t6k77xsvgeas4eaesj35f4ws6qy6ptwd/ -Denable_BUFFER_SIZE=no ../CDMlib && make VERVOSE=1 && make install
...
function build_polylib {
	...
	CXX=${cxx_compiler} CC=${c_compiler} CFLAGS=${c_flags} CXXFLAGS=${cxx_flags} ${CMAKE_BIN} -DCMAKE_TOOLCHAIN_FILE=../Polylib/cmake/Toolchain_Fugaku.cmake -DINSTALL_DIR=${installdir}/Polylib -Dwith_MPI=yes -Dwith_TP=${installdir}/TextParser -Dreal_type=float -Dwith_example=no ../Polylib && make && make install
	if [[ $? != 0 ]]; then exit $?; fi
	cd ${topdir}
...
function build_bcmtools {
	...
	CXX=${cxx_compiler} CC=${c_compiler} CFLAGS=${c_flags} CXXFLAGS=${cxx_flags} ${CMAKE_BIN} -DCMAKE_TOOLCHAIN_FILE=../BCMTools/cmake/Toolchain_Fugaku.cmake -DINSTALL_DIR=${installdir}/BCMTools -Denable_OPENMP=yes -Dwith_MPI=yes -Dwith_example=no -Dwith_TP=${installdir}/TextParser -Dwith_PL=${installdir}/Polylib -Dreal_type=float -Denable_LARGE_BLOCK=no ../BCMTools && make && make install
...
function build_compositor {
	...
	CXX=${cxx_compiler} CC=${c_compiler} CXXFLAGS="${cxx_flags} -Kopenmp" CFLAGS="${c_flags} -Kopenmp" ./configure --prefix=${installdir}/234Compositor &&  make && make install
	if [[ $? != 0 ]]; then exit $?; fi
...
function build_cpmlib {
	...
	${CMAKE_BIN} -DINSTALL_DIR=${installdir}/CPMlib -DCMAKE_TOOLCHAIN_FILE=../CPMlib/cmake/Toolchain_Fugaku.cmake -Dwith_MPI=yes -Dreal_type=float -Denable_LMR=no -Dwith_example=no -Dwith_TP=${installdir}/TextParser ../CPMlib && make && make install
        if [[ $? != 0 ]]; then exit $?; fi
...
# nanomsg is not supported on Fugaku platform
#build_nanomsg
